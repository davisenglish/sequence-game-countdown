...
export default function WordPuzzleGame() {
  ...
  const [timeLeft, setTimeLeft] = useState(LETTER_TIME_LIMIT);
  ...
  const handleSubmit = async (e) => {
    ...
    const timeBonus = timeLeft >= LETTER_TIME_LIMIT - 10 ? 5 : 0;
    if (firstAnswerTime === null) {
      const elapsed = (performance.now() - startTimeRef.current) / 1000;
      setFirstAnswerTime(elapsed);
    }
    // Store word, its length, and bonusTime for clarity
    setValidWords(prev => [...prev, { word, length: word.length, bonusTime: timeBonus }]);
  ...
  const resetGame = () => {
    ...
    setError(false); setErrorMessage(''); setTimeLeft(LETTER_TIME_LIMIT);
    setGameOver(false); setLetterPopup(null); setTimePopup(null); setFirstAnswerTime(null);
  };
  ...
  const updateStats = () => {
    ...
    newStats.fastestAnswers = newStats.fastestAnswers || [];
    ...
    // Update fastest answers
    if (firstAnswerTime !== null) {
      newStats.fastestAnswers.push({
        time: firstAnswerTime,
        word: validWords[0]?.word || 'Unknown'
      });
      newStats.fastestAnswers.sort((a, b) => a.time - b.time); // Sort ascending
      newStats.fastestAnswers = newStats.fastestAnswers.slice(0, 3); // Keep top 3
    }
  ...
  return (
    <div className="p-6 max-w-xl mx-auto text-center space-y-6 relative overflow-hidden">
      ...
      {roundStarted && !gameOver && <div className={`text-xl font-semibold ${timeLeft>0 && timeLeft<=10?'text-red-800 animate-time-shake':timeLeft>0?'text-black':'text-red-800'} ${showRevealAnimation ? 'reveal-content' : ''}`}>Time Left: <span className="font-bold">{timeLeft}s</span></div>}
              ...
              <div className={`absolute left-1/2 transform -translate-x-1/2 w-32 bg-gray-300 rounded-full h-6 opacity-60 transition-opacity duration-300 overflow-hidden ${gameOver || timeLeft < LETTER_TIME_LIMIT - 10 ? 'opacity-0' : ''}`}>
                <div 
                  ...
                  style={{
                    ...
                    opacity: 0.6,
                    width: `${Math.max(0, Math.min(100, ((timeLeft - (LETTER_TIME_LIMIT - 10)) / 10) * 100))}%`
                  }}
                ></div>
              </div>
            ...
            {gameOver && firstAnswerTime !== null && (
              ...
              {showAllWords ? (
                ...
                <div className="flex flex-wrap justify-center gap-2 max-w-md">
                  {validWords.slice().reverse().map(({word,length,bonusTime},idx)=>(
                ...
                <div className="flex flex-wrap justify-center gap-2 max-w-md">
                  {validWords.slice(-3).reverse().map(({word,length,bonusTime},idx)=>(
                    <div key={idx} className="rounded-lg px-3 py-1 flex items-center space-x-1" style={{backgroundColor: 'rgba(28, 109, 42, 0.15)', border: '1px solid rgba(28, 109, 42, 0.3)'}}>
                      <span className="font-medium text-sm" style={{color: '#1c6d2a'}}>{word}</span>
                      <span className="text-xs" style={{color: '#1c6d2a'}}>({length})</span>
                  {bonusTime>0 && (
      ...
      {showStats && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] modal-fade-in" style={{ top: '-100vh', left: '-100vw', right: '-100vw', bottom: '-100vh', width: '300vw', height: '300vh' }}>
          <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-xs sm:max-w-sm md:max-w-md mx-4 sm:mx-6 max-h-[85vh] sm:max-h-[90vh] overflow-y-auto">
            ...
            {/* Fastest Answers and Highest Word Count */}
            <div className="grid grid-cols-2 gap-4 sm:gap-6">
              {/* Fastest Answers */}
              <div>
                <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-left">Fastest Answers</h3>
                <div className="space-y-2">
                  {[1, 2, 3].map((position) => {
                    ...
                    return (
                      <div key={position} className="flex items-center space-x-3">
                        ...
                        >
                          {answer ? `${answer.time.toFixed(3)} seconds (${answer.word})` : '-'}
                        </span>
                      </div>
                    );
                  })}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] modal-fade-in" style={{ top: '-100vh', left: '-100vw', right: '-100vw', bottom: '-100vh', width: '300vw', height: '300vh' }}>
          ...
          >
            ...
            <ul className="mb-3 text-sm list-disc pl-5 space-y-1">
              ...
              <li>+1 point per letter in each word & +5 point time bonus for answers in the first 10 seconds.</li>
            </ul>
